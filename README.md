...
actuator-viz computes the **effectiveness matrix** from your actuator geometry and visualizes what it means. Here's the output for the included 8-thruster ROV example:

![3D actuator layout with thrust vectors](docs/images/layout_3d.png)

The **effectiveness matrix** shows exactly which actuators drive which degrees of freedom—and where forces couple:

![Effectiveness matrix heatmap](docs/images/effectiveness_heatmap.png)

```
✓ Full rank (6/6 DOF controllable)
✓ Condition number: 6.2 (well-conditioned)
```

Reports are self-contained interactive HTML—open them offline, share via email or Slack.
...
Want the real vehicle body in the 3D view instead of a bounding box? Point `geometry` at an STL/OBJ mesh and install the mesh extra (`pip install 'actuator-viz[mesh]'`)—see `examples/torpedo_auv.yaml`.
