...
def test_output_report_is_self_contained(examples_dir, tmp_path):
    ...
    report = tmp_path / "report.html"
    runner.invoke(app, [str(examples_dir / "rov_8_thruster.yaml"), "--output", str(report)])
...
def test_failure_all_reports_and_stays_zero_for_tolerant_rov(examples_dir):
    result = runner.invoke(app, [str(examples_dir / "rov_8_thruster.yaml"), "--failure-all"])
    assert result.exit_code == 0
...
def test_failure_invalid_id_exits_one(examples_dir):
    result = runner.invoke(app, [str(examples_dir / "rov_8_thruster.yaml"), "--failure", "99"])
    assert result.exit_code == 1
